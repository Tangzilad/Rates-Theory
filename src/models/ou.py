"""Ornstein-Uhlenbeck engines used in Chapter 2 labs.

Pedagogical simplification:
- Uses Euler discretization and simple AR(1) mapping for parameter estimation.
- Appropriate for educational diagnostics, not production calibration.
"""
...
def simulate_ou(
    ...
    """Simulate OU paths via Euler discretization.
...
def estimate_ou_parameters(series: Iterable[float], dt: float = 1.0 / TRADING_DAYS_PER_YEAR) -> OUParameters:
    """Estimate OU parameters from an AR(1) mapping."""
...
def conditional_expectation(x_t: float, horizon: float, theta: float, mu: float) -> float:
...
def sharpe_ratio(returns: Iterable[float], risk_free_rate: float = 0.0, annualize: bool = True) -> float:
    """Compute (optional annualized) Sharpe ratio."""
...
def first_passage_time_approx(x0: float, level: float, theta: float, mu: float, sigma: float) -> float:
    """Approximate expected first-passage time for OU.

    Pedagogical simplification: uses an effective-drift heuristic for ranking,
    rather than solving exact boundary-value formulas.
    """
